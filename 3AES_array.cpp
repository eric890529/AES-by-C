...
void setKey0() {
	printf("請輸入金鑰：\n");
	fgets(inputKey[0],  sizeof(inputKey[0]), stdin);
	system("cls");
	return;
}//setKey0
...
void setKey1() {
	printf("請輸入金鑰：\n");
	fgets(inputKey[1],  sizeof(inputKey[1]), stdin);
	system("cls");
	return;
}//setKey
...
void setKey2() {
	printf("請輸入金鑰：\n");
	fgets(inputKey[2],  sizeof(inputKey[2]), stdin);
	system("cls");
	return;
}//setKey
		...
		switch(encryption_decryption) {
			case 1:
				while(true) {
					...
					printf("3. 金鑰0(超過%d Bits的部分不會被使用)：\n%s\n\n", keyBits, inputKey[0]);
					printf("4. 金鑰1(超過%d Bits的部分不會被使用)：\n%s\n\n", keyBits, inputKey[1]);
					printf("5. 金鑰2(超過%d Bits的部分不會被使用)：\n%s\n\n", keyBits, inputKey[2]);
					printf("6. 欲加密文字：\n%s\n\n", inputPlainText);
					printf("7. 初始向量(IV)：\n");
					for(int i=0; i<4; i++)
						for(int j=0; j<4; j++) 
			...
			case 2:
				while(true) {
					...
					printf("4. 金鑰(超過%d Bits的部分不會被使用)：\n%s\n\n", keyBits, inputKey[1]);
					printf("5. 金鑰(超過%d Bits的部分不會被使用)：\n%s\n\n", keyBits, inputKey[2]);
					printf("6. 欲解密16進位個數：%d\n\n", cipherTextNum);
					for(int i=0; i<cipherTextNum; i++)
...
void de_CFB_8(uint8_t cipherText_blocks[][4][4], uint8_t plainText_blocks[][4][4], uint32_t* keys) {
	...
	if(encryption_decryption == 2) inputLength = cipherTextNum;
	int times=(inputLength*8)/s;
	for(int i=0,  y=0,  x=0; i<times; i++) {
		uint8_t temp[4][4];
		for(int j=0; j<4; j++)
...
void de_OFB_8(uint8_t cipherText_blocks[][4][4], uint8_t plainText_blocks[][4][4], uint32_t* keys) {
	...
	if(encryption_decryption == 1) breakTime = inputLength;
	else breakTime = cipherTextNum;
...
int main() {//47 26 fa 34 9 96 d0 8 3a 5a b9 8 35 a8
	uint8_t*** key;
	key = (uint8_t***)calloc(3, sizeof(uint8_t**));
	for(int i=0; i<3; i++)
		key[i] = (uint8_t**)calloc(4, sizeof(uint8_t*));
	mod = 1;
	keyBits = 128;
	...
	while(true) {
		strcpy(inputKey[0], "Thats my Kung FuThats my Kung Fu\n");
		strcpy(inputKey[1], "Thats my Kung FuThats my Kung Fu\n");
		strcpy(inputKey[2], "Thats my Kung FuThats my Kung Fu\n");
		for(int i=0; i<4; i++)
			for(int j=0; j<4; j++)
				IV[j][i] = 0x61;
		...
		for(int i=0; i<3; i++)
			for(int j=0; j<4; j++)
				key[i][j] = (uint8_t*)calloc(Nk, sizeof(uint8_t));
				
		uint32_t keys[3][keyNum];
		...
		for(int i=0; i<3; i++) {
			...
			for(int j=0; j<Nk; j++)
				for(int k=0; k<4; k++){
					key[i][k][j] = inputKey[i][count];
					count++;
				}//for
		}//for
		
		for(int i=0; i<3; i++)
			keyExpansion(key[i], keys[i], Nk, keyNum);
		...
		if(encryption_decryption == 1) {
			useKeyCount = 0;
			if(mod == 1) {	//ECB
				ECB(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				useKeyCount++;
				printf("\n");
				de_ECB(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				printf("result:\n");
				ECB(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
			} else if(mod == 2) {	//CBC
				CBC(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				de_CBC(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				printf("result:\n");
				CBC(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
			} else if(mod == 3) {	//CFB-8
				CFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				de_CFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				printf("\nresult:\n");
				CFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
			} else if(mod == 4) {	//OFB
				OFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				de_OFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				printf("\nresult:\n");
				OFB_8(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
			} else if(mod == 5) {	//CTR
				CTR(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				de_CTR(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
				printf("\n");
				useKeyCount++;
				printf("\nresult:\n");
				CTR(cipherText_blocks, plainText_blocks, keys[useKeyCount]);
...
//Fuck you bitch Fuck you bitch yo
//mother funcking AES so hard fuck
//82 FC A6 F5 9F C4 33 C9 44 1F DC 3C 20 BB BE 82
//47 26 FA 34 09 96 D0 08 3A 5A B9 08 35 A8 39 CC
